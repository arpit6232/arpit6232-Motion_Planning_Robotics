...
"""
Gradient Based Calculation explained here at: 
1) A. Kelly and B. Nagy, “Reactive Nonholonomic Trajectory Generation
via Parametric Optimal Control,” The International Journal of Robotics
Research, vol. 22, no. 7, pp. 583–601, 2003. This paper discusses the
math behind generating spirals to desired terminal states.
...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        PARAMETERS
        ----------
            xf, yf, tf: Final pose for the given goal state
        RETURNS
        -------
        ...
        # The most basic spiral a straight line
        sf_0 = np.linalg.norm([xf, yf])
        # Spiral Params, set p0 and p4 (being the curvature of
        ...
        p = res.x
        p = [0.0, p[0], p[1], 0.0, p[2]]    # p0 and p3 are set to 0
        a = p[0]
        ...
        # List of s values along the spiral
        s_points = np.linspace(0.0, p[4])

        t_points = [a * x + b * x**2 / 2 + c * x**3 / 3 + d * x**4 / 4 for x in s_points]
        ...
        spiral = [x_points, y_points, t_points]
    ...
    def fxf(self, p):
        """
        Cost Calculation helper along x direction, hardcoded for optimum spiral results. (As per Literature)
        """
        t2 = p[0]*(1.1E1/2.0);
        t3 = p[1]*9.0;
        t4 = p[2]*(9.0/2.0);
    ...
    def fxf_grad(self, p):
        """
        Gradient Calculation helper along x direction, hardcoded for optimum spiral results. (As per Literature)
        """
        grad = [0.0, 0.0, 0.0]
    ...
    def fyf(self, p):
        """
        Cost Calculation helper along y direction, hardcoded for optimum spiral results. (As per Literature)
        """
        t2 = p[0]*(1.1E1/2.0);
        t3 = p[1]*9.0;
        t4 = p[2]*(9.0/2.0);
    ...
    def fyf_grad(self, p):
        """
        Gradient Calculation helper along y direction, hardcoded for optimum spiral results. (As per Literature)
        """
        grad = [0.0, 0.0, 0.0]
    ...
    def ftf(self, p):
        """
        Cost Calculation helper for yaw, hardcoded for optimum spiral results. (As per Literature)
        """
    ...
    def ftf_grad(self, p):
        """
        Gradient Calculation helper for yaw, hardcoded for optimum spiral results. (As per Literature)
        """
        grad = [0.0, 0.0, 0.0]
